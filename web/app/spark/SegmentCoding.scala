...
object SegmentCoding {
  ...
  def coding(description: String, kraFilePath: String, configFile: Stream[Array[String]], catFile: List[Array[String]]): List[String] = {

    val category = description.split(",")(0)
    var KRAFile = List[Array[String]]()
    var cateCodeCombine = ""
    var segNoCombine = ""
    if (category == "BIS") {
      KRAFile = scala.io.Source.fromFile(kraFilePath)(Codec.UTF8).getLines().map(_.split(",")).toList
    }
    if (category == "SKIN") {
    ...
    val configFileT = configFile.filter(_ (1) == category).toList
    val text = transCateCode(description, catFile).split(",")

    val seglist = configFileT.filter(x => x(3) != "BRAND" && !x(3).contains("SUBBRAND") && x(3) != "PACKSIZE" && x(3) != "PRICETIER" && x(3) != "CATEGORY")
      .map(_ (3)).distinct

    val subbrand_namelist = configFileT.filter(_ (3).contains("SUBBRAND")).map(_ (3)).distinct

    val tempre = segmentCoding(text, configFileT, seglist, subbrand_namelist, "ALL", KRAFile, cateCodeCombine, segNoCombine, "TMTB")

    tempre._2
  }

  def segmentCoding(item_raw: Array[String], configFile: List[Array[String]], seglist: List[String], subbrand_namelist: List[String], codingType: String, krafile: List[Array[String]], cateCodeCombine: String, segCombine: String, t: String): (item, List[String]) = {
